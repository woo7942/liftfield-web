  ...
  const checkInviteCode = async () => {
    ...
    try {
      ...
      if (data.expireAt?.toDate() < now) {
        setInviteMsg('❌ 만료된 초대코드예요.');
        setInviteInfo(null);
        return;
      }
      if (data.usedCount >= data.maxMembers) {
        setInviteMsg(`❌ 인원이 가득 찼어요. (${data.usedCount}/${data.maxMembers}명)`);
        setInviteInfo(null);
        return;
      }
      setInviteInfo({
      ...
      setInviteMsg(
        `✅ 확인됐어요! [${data.teamName}] 팀 (${data.usedCount + 1}/${data.maxMembers}번째 멤버)`
      );
    } catch {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, 'users', uid), {
        ...
        maxMembers: selectedPlan === 'company' ? maxMembers : 1,
          ...
          {selectedPlan === 'company' && !inviteMode && (
            <div className="bg-purple-50 rounded-xl p-4 my-4 text-left">
              ...
              <p className="text-xs text-purple-600">
                로그인 후 팀 관리 메뉴에서 팀별 초대코드를 발급하세요.
              </p>
            </div>
          )}
        ...
        <div className="px-8 py-6">
              ...
              <div className="border border-blue-100 rounded-xl p-4 bg-blue-50">
                ...
                {inviteMode && (
                  <div className="mt-3 space-y-2">
                    ...
                    {inviteMsg && (
                      <p className="text-xs text-gray-600">{inviteMsg}</p>
                    )}
                  </div>
                )}
              </div>
          ...
          {step === 3 && (
            <div className="space-y-4">
                ...
                {inviteMode && inviteInfo ? (
                  <>
                    <p>
                      ...
                      <span className="font-medium text-blue-600">초대코드</span>
                    </p>
                    <p>
                    ...
                    {selectedPlan === 'company' && (
                      <>
                        ...
                        <p>
                          ...
                          <span className="font-medium">{selectedMembers}명</span>
                        </p>
                      </>
                    )}
                  </>
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => setStep(inviteMode && inviteInfo ? 1 : 2)}
                  className="flex-1 border border-gray-300 text-gray-600 py-3 rounded-xl font-semibold hover:bg-gray-50 transition"
                >
                  이전
                </button>
