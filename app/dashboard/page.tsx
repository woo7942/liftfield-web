...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const u1 = onSnapshot(faultQ, s => setCounts(p => ({ ...p, fault: s.size })));
  ...
  const expiryTop = [...sites]
    ...
    .sort((a, b) => {
      const da = getDday(a.contractEnd) ?? 9999;
      const db_ = getDday(b.contractEnd) ?? 9999;
      return da - db_;
    })
    .slice(0, 5);
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: '10px 12px' }}>
                ...
                <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid #f1f5f9' }}>
                  ...
                  {expiryTop.length === 0 ? (
                    ...
                    if (!info) return null;
                    const typeColor = s.contractType?.includes('종합') ? { bg: '#eff6ff', color: '#3b82f6' } : { bg: '#f5f3ff', color: '#7c3aed' };
                    return (
